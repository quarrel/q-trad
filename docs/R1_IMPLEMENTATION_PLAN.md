...
\* native `QUOTE\_DERIVED` data only;
...
`dataset\_id` identifies the meaning and contents of a dataset.
...
Rebuilding identical semantic content from reordered inputs must produce the same `dataset\_id`.
...
`manifest\_id` identifies one immutable physical representation.
...
Two builds may have the same `dataset\_id` but different `manifest\_id` values.
...
The observation dataset stores both `received\_at` and `persisted\_at`.
...
availability\_basis = "persisted\_at"

```



`persisted\_at` is the initial decision-grade default because downstream research consumes persisted canonical events.



No derived `available\_at` field should obscure its source. Consumers calculate:
...
&#x20;   persisted\_at when availability\_basis == "persisted\_at"

&#x20;   received\_at  when availability\_basis == "received\_at"
...
observation\_dataset\_id

ordered\_instruments
...
feature\_lag\_policy

feature\_lag\_calibration\_range

feature\_lag\_percentile

feature\_lag\_safety\_margin

selected\_feature\_lag



target\_horizons

primary\_vertical\_horizon

target\_revision\_delay



required\_feature\_bases
...
minimum\_training\_duration

minimum\_validation\_duration
...
availability\_time - interval\_end
...
&#x20;   ceil\_to\_whole\_grid\_unit(

&#x20;       configured\_percentile\_of\_eligible\_delays

&#x20;       + configured\_safety\_margin
...
feature\_data\_asof = decision\_time - selected\_feature\_lag

latest\_feature\_bar\_end = feature\_data\_asof
...
interval\_end == latest\_feature\_bar\_end
...
availability\_time <= feature\_data\_asof
...
highest stream\_version

then highest global\_position
...
selected\_stream\_version

selected\_global\_position

selected\_availability\_time
...
EVENTUALLY\_OBSERVED\_LATE

RECORDED\_GAP\_KNOWN\_BY\_CUTOFF

RECORDED\_GAP\_DETECTED\_LATER

SOURCE\_NOT\_ACTIVE

NO\_NATIVE\_EVIDENCE

AMBIGUOUS\_OR\_INVALID\_SOURCE
...
gap.detected\_at <= feature\_data\_asof
...
The panel `dataset\_id` binds:



\* observation `dataset\_id`;
...
target\_start\_time = decision\_time

target\_end\_time = decision\_time + H
...
log(label\_end\_close / label\_start\_close)
...
target\_freeze\_at = target\_end\_time + target\_revision\_delay

```



Each required target bar uses the newest revision available by `target\_freeze\_at`.



Revisions arriving after `target\_freeze\_at` remain in the observation dataset but do not mutate that target dataset.
...
NON\_POSITIVE\_START

NON\_POSITIVE\_END

AMBIGUOUS\_SOURCE

UNAVAILABLE\_BY\_FREEZE
...
upper\_log\_excursion

lower\_log\_excursion

```



These are relative to `label\_start\_close`.
...
target\_available\_at = target\_freeze\_at
...
decision\_time < training\_cutoff

target\_available\_at <= training\_cutoff

return\_disposition == VALID
...
target\_end\_time <= training\_cutoff
...
validation\_start <= decision\_time < validation\_end
...
Do not trust a caller-supplied Boolean `is\_oof`.
...
model\_contract = "DETERMINISTIC\_PROBE"

model\_id = hash("zero-return-v1")

expected\_return = 0.0
...
foundation\_configuration

availability\_delay\_report
...
\* `received\_at` and `persisted\_at` remain distinct.

\* A projection revision without exactly one canonical event fails.

\* Reversing input order preserves `dataset\_id`.

\* Rebuilding at another clock time preserves `dataset\_id` but changes physical manifest identity.
...
\* A later-arriving bar remains `MISSING\_AS\_OF\_CUTOFF` causally and is diagnosed separately as late.
...
3\. Read `docs/TRADING\_RESEARCH.md`.
