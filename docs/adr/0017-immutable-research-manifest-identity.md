...
capture-universe name and configuration hash, row and time bounds, semantic bar-content hash,
...
Parquet partitions are written exclusively under `bars-v2/` and are never replaced. An existing
...
distinct immutable manifests while safely sharing unchanged content-addressed partitions. Byte or
metadata tampering fails replay. Forward schema application remains compatible with application
