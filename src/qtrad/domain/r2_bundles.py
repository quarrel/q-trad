...
@dataclass(frozen=True, slots=True)
class R2SoftwareVerificationBundle:
    ...
    CONTRACT: ClassVar[str] = R2_SOFTWARE_VERIFICATION_CONTRACT
    SCHEMA_VERSION: ClassVar[int] = 1
    ...
    def __post_init__(self) -> None:
        ...
        if any(item.path == "manifest.json" for item in references):
            raise ValueError("software bundle child path is reserved")
        if len({item.path for item in references}) != len(references):
            raise ValueError("software bundle children must not contain duplicate paths")
        if len({(item.contract, item.semantic_id) for item in references}) != len(references):
            raise ValueError("software bundle children must not contain duplicate identities")
        identities = (
        ...
        if self.representative_integration_ready != "READY":
            raise ValueError("software verification requires representative integration readiness")
        if (
            self.evidence_disposition != "IMPLEMENTATION_EVIDENCE_ONLY"
            or self.research_disposition != "RESEARCH_EVIDENCE_PENDING"
        ):
            raise ValueError("software verification dispositions are fixed until R2.F2")
        if self.bundle_id != _semantic_id(self.semantic_json()):
            raise ValueError("software bundle ID does not authenticate its content")

    @classmethod
    def create(cls, **values: Any) -> R2SoftwareVerificationBundle:
        semantic = {
            "contract": cls.CONTRACT,
            "schema_version": cls.SCHEMA_VERSION,
            **{
    ...
    @classmethod
    def from_json(cls, value: object) -> R2SoftwareVerificationBundle:
        if not isinstance(value, dict):
